...
const GamePanelWidget = ({ isAuth, isVkGuest, profileUser }) => {
  ...
  const percentage = Math.min(100, Math.max(0, (currentEnergy / allowed) * 100))
  ...
  return (
    <div className={`${styles.game_panel_capsule} ${(!isPremium && isEmpty) ? styles.pulse_alert : ''}`}>
      ...
      >
        <div className={styles.gamification_section} onClick={() => navigate(isAuth ? '/dashboard' : '/auth')}>
          <span className={`${styles.status_text} ${styles.level_full}`}>Lvl {currentLevel}</span>
          <span className={`${styles.status_text} ${styles.level_short}`}>Lvl {currentLevel}</span>
