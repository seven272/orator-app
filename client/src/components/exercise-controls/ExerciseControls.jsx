...
const ExerciseControls = ({
  ...
  const handleStart = () => {
    ...
    // БЛОКИРОВКА ПРИ ПУСТОМ БАКЕ ЭНЕРГИИ
    if (availableEnergy < cost) {
      if (isVkEnvironment) {
        // В среде ВК вместо окна Premium выкидываем «Центр бонусов» для бесплатной зарядки бака
        dispatch(openViralModal())
      } else {
        // На обычном внешнем сайте открываем стандартный Premium Offer
  ...
  const handleShareStory = async () => {
    ...
    if (result && result.success) {
      message.success({
        content: 'История успешно добавлена!',
        key: 'storyVkKey',
        duration: 3,
      })
    } else {
