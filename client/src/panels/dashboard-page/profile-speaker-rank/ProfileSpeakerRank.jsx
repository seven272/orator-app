...
const ProfileSpeakerRank = ({ userXp, progressData }) => {
  ...
  const totalRoadPercent = Math.min((userXp / 500000) * 100, 100)
    ...
    <div className={styles.mic_progress_block}>
      ...
      <div className={styles.mic_header}>
        ...
        <div className={styles.mic_titles}>
          ...
          <h5 className={styles.audience_text}>{progressData.title}</h5>
        </div>
      </div>
      ...
      <div className={styles.xp_wrapper}>
        ...
        <span className={styles.audience_sub_hint}>🔥 {progressData.audience}</span>
      </div>
              ...
              return (
                ...
                >
                  <div className={styles.node_mic_box}>
                    <span className={styles.node_emoji}>{stage.mic}</span>
                  </div>
                  <span className={styles.node_title}>{stage.title}</span>
                  <span className={styles.node_xp_hint}>
                    {stage.xpThreshold === 0 ? '0 XP' : `${stage.xpThreshold / 1000}к+ XP`}
                  </span>
                </div>
              )
            })}
