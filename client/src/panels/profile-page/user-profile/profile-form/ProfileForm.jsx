...
const ProfileForm = () => {
  ...
  const handleUpdateProfile = async (evt) => {
    ...
    try {
      ...
      await dispatch(fetchUpdateProfile({ 
        firstName, 
        lastName, 
        displayName, 
        email, 
  ...
  const isEmailFieldsVisible = !user?.vkId || (user?.vkId && user?.email)
    ...
    <form onSubmit={handleUpdateProfile} className={styles.form_profile}>
      ...
      <div className={styles.avatar_edit_section}>
        <div className={styles.preview_box}>
          ...
          {imgUrl ? (
            <img src={imgUrl} alt="Avatar Preview" className={styles.avatar_preview_img} />
          ) : (
